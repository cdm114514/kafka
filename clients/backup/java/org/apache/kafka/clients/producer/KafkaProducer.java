...
public class KafkaProducer<K, V> implements Producer<K, V>
{
    ...
    KafkaProducer(Map<String, Object> configs, Serializer<K> keySerializer, Serializer<V> valueSerializer, ProducerMetadata metadata, KafkaClient kafkaClient, ProducerInterceptors<K, V> interceptors, Time time)
    {
        ...
        try
        {
            ...
            List<ProducerInterceptor<K, V>> interceptorList = (List) configWithClientId.getConfiguredInstances(ProducerConfig.INTERCEPTOR_CLASSES_CONFIG, ProducerInterceptor.class);
    ...
    private TransactionManager configureTransactionState(ProducerConfig config, LogContext logContext)
    {
        ...
        {
            log.info("Overriding the default {} to true since {} is specified.", ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG, ProducerConfig.TRANSACTIONAL_ID_CONFIG);
        }
        ...
        if (config.idempotenceEnabled())
        {
            ...
            transactionManager = new TransactionManager(logContext, transactionalId, transactionTimeoutMs, retryBackoffMs, apiVersions, autoDowngradeTxnCommit);
    ...
    private static short configureAcks(ProducerConfig config, Logger log)
    {
        ...
        if (config.idempotenceEnabled())
        {
            ...
            {
                throw new ConfigException("Must set " + ProducerConfig.ACKS_CONFIG + " to all in order to use the idempotent " + "producer. Otherwise we cannot guarantee idempotence.");
            }
        }
        return acks;
    }
