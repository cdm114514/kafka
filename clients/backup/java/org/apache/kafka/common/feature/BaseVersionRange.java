...
class BaseVersionRange
{
    ...
    {
        return String.format("%s[%s]", this.getClass().getSimpleName(), mapToString(toMap()));
    }
    ...
    {
        return map.entrySet().stream().map(entry -> String.format("%s:%d", entry.getKey(), entry.getValue())).collect(joining(", "));
    }
