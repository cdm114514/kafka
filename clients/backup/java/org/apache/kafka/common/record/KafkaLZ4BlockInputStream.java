...
public final class KafkaLZ4BlockInputStream extends InputStream
{
    ...
    private void readHeader() throws IOException
    {
        ...
        int hash = in.hasArray() ?
                ...
                CHECKSUM.hash(in.array(), in.arrayOffset() + in.position(), len, 0) : CHECKSUM.hash(in, in.position(), len, 0);
    ...
    private void readBlock() throws IOException
    {
        ...
        if (compressed)
        {
            try
            {
                ...
                if (in.hasArray())
                {
                    bufferSize = DECOMPRESSOR.decompress(in.array(), in.position() + in.arrayOffset(), blockSize, decompressionBuffer.array(), 0, maxBlockSize);
                } else
                {
        ...
        {
            // workaround for https://github.com/lz4/lz4-java/pull/65
            int hash = in.hasArray() ? CHECKSUM.hash(in.array(), in.arrayOffset() + in.position(), blockSize, 0) : CHECKSUM.hash(in, in.position(), blockSize, 0);
