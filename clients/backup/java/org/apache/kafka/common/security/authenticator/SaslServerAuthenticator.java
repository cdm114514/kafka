...
public class SaslServerAuthenticator implements Authenticator
{
    ...
    public SaslServerAuthenticator(Map<String, ?> configs, Map<String, AuthenticateCallbackHandler> callbackHandlers, String connectionId, Map<String, Subject> subjects, KerberosShortNamer kerberosNameParser, ListenerName listenerName, SecurityProtocol securityProtocol, TransportLayer transportLayer, Map<String, Long> connectionsMaxReauthMsByMechanism, ChannelMetadataRegistry metadataRegistry, Time time)
    {
        ...
        for (String mechanism : this.enabledMechanisms)
        {
            ...
            LOG.debug("{} for mechanism={}: {}", BrokerSecurityConfigs.CONNECTIONS_MAX_REAUTH_MS, mechanism, connectionsMaxReauthMsByMechanism.get(mechanism));
        }
    ...
    public void reauthenticate(ReauthenticationContext reauthenticationContext) throws IOException
    {
        ...
        reauthInfo.reauthenticating(previousSaslServerAuthenticator.saslMechanism, previousSaslServerAuthenticator.principal(), reauthenticationContext.reauthenticationBeginNanos());
    ...
    private void handleSaslToken(byte[] clientToken) throws IOException
    {
        ...
        } else
        {
            ...
            RequestContext requestContext = new RequestContext(header, connectionId, clientAddress(), KafkaPrincipal.ANONYMOUS, listenerName, securityProtocol, ClientInformation.EMPTY);
            ...
            } catch (SaslException e)
            {
                ...
                } else
                {
                    ...
                    sendKafkaResponse(requestContext, new SaslAuthenticateResponse(new SaslAuthenticateResponseData().setErrorCode(Errors.SASL_AUTHENTICATION_FAILED.code()).setErrorMessage(errorMessage)));
    ...
    private class ReauthInfo
    {
        ...
        public void ensurePrincipalUnchanged(KafkaPrincipal reauthenticatedKafkaPrincipal) throws SaslAuthenticationException
        {
            ...
            {
                throw new SaslAuthenticationException(String.format("Cannot change principals during re-authentication from %s.%s: %s.%s", previousKafkaPrincipal.getPrincipalType(), previousKafkaPrincipal.getName(), reauthenticatedKafkaPrincipal.getPrincipalType(), reauthenticatedKafkaPrincipal.getName()));
            }
        }
        ...
        public boolean saslMechanismUnchanged(String clientMechanism)
        {
            ...
            badMechanismErrorMessage = String.format("SASL mechanism '%s' requested by client is not supported for re-authentication of mechanism '%s'", clientMechanism, previousSaslMechanism);
        ...
        private long calcCompletionTimesAndReturnSessionLifetimeMs()
        {
            ...
            if (credentialExpirationMs != null)
            {
                ...
                {
                    LOG.debug("Authentication complete; session max lifetime from broker config={} ms, credential expiration={} ({} ms); no session expiration, sending 0 ms to client", connectionsMaxReauthMs, new Date(credentialExpirationMs), credentialExpirationMs - authenticationEndMs);
                }
            } else
            {
                if (sessionExpirationTimeNanos != null)
                {
                    LOG.debug("Authentication complete; session max lifetime from broker config={} ms, no credential expiration; session expiration = {} ({} ms), sending {} ms to client", connectionsMaxReauthMs, new Date(authenticationEndMs + retvalSessionLifetimeMs), retvalSessionLifetimeMs, retvalSessionLifetimeMs);
                } else
                {
                    LOG.debug("Authentication complete; session max lifetime from broker config={} ms, no credential expiration; no session expiration, sending 0 ms to client", connectionsMaxReauthMs);
                }
            }
