...
public final class IntGaugeSuite<K> implements AutoCloseable
{
    ...
    public IntGaugeSuite(Logger log, String suiteName, Metrics metrics, Function<K, MetricName> metricNameCalculator, int maxEntries)
    {
        ...
        this.closed = false;
        log.trace("{}: created new gauge suite with maxEntries = {}.", suiteName, maxEntries);
    }
    ...
    public void increment(K key)
    {
        synchronized (this)
        {
            if (closed)
            {
                log.warn("{}: Attempted to increment {}, but the GaugeSuite was closed.", suiteName, key.toString());
                return;
            }
            ...
            if (gauges.size() == maxEntries)
            {
                ...
                {
                    log.debug("{}: Attempted to increment {}, but there are already {} entries.", suiteName, key.toString(), maxEntries);
                    return;
                }
                ...
                log.trace("{}: Removing the metric {}, which has a value of 0.", suiteName, keyToRemove.toString());
            }
    ...
    private void performPendingMetricsOperations()
    {
        ...
        try
        {
            ...
            for (PendingMetricsChange change = pending.pollLast(); change != null; change = pending.pollLast())
            {
    ...
    public synchronized void decrement(K key)
    {
        if (closed)
        {
            log.warn("{}: Attempted to decrement {}, but the gauge suite was closed.", suiteName, key.toString());
            return;
        }
        StoredIntGauge gauge = gauges.get(key);
        if (gauge == null)
        {
            log.debug("{}: Attempted to decrement {}, but no such metric was registered.", suiteName, key.toString());
        } else
        {
            int cur = gauge.decrement();
            log.trace("{}: Removed a reference to {}.  {} reference(s) remaining.", suiteName, key.toString(), cur);
            if (cur <= 0)
            {
                removable.add(key);
            }
