...
public class LeaveGroupRequest extends AbstractRequest
{
    ...
    public static class Builder extends AbstractRequest.Builder<LeaveGroupRequest>
    {
        ...
        public LeaveGroupRequest build(short version)
        {
            ...
            if (version >= 3)
            {
                data = new LeaveGroupRequestData().setGroupId(groupId).setMembers(members);
            } else
            {
                if (members.size() != 1)
                {
                    throw new UnsupportedVersionException("Version " + version + " leave group request only " + "supports single member instance than " + members.size() + " members");
                }

                data = new LeaveGroupRequestData().setGroupId(groupId).setMemberId(members.get(0).memberId());
            }
