...
public class AlterReplicaLogDirsRequest extends AbstractRequest
{
    ...
    public Map<TopicPartition, String> partitionDirs()
    {
        ...
        data.dirs().forEach(alterDir -> alterDir.topics().forEach(topic -> topic.partitions().forEach(partition -> result.put(new TopicPartition(topic.name(), partition.intValue()), alterDir.path()))));
        return result;
    }
