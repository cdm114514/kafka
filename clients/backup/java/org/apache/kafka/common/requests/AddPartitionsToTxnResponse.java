...
public class AddPartitionsToTxnResponse extends AbstractResponse
{
    ...
    public AddPartitionsToTxnResponse(int throttleTimeMs, Map<TopicPartition, Errors> errors)
    {
        ...
        for (Map.Entry<TopicPartition, Errors> entry : errors.entrySet())
        {
            ...
            AddPartitionsToTxnPartitionResult partitionResult = new AddPartitionsToTxnPartitionResult().setErrorCode(entry.getValue().code()).setPartitionIndex(topicPartition.partition());
        ...
        {
            topicCollection.add(new AddPartitionsToTxnTopicResult().setName(entry.getKey()).setResults(entry.getValue()));
        }
    ...
    public Map<TopicPartition, Errors> errors()
    {
        ...
        for (AddPartitionsToTxnTopicResult topicResult : this.data.results())
        {
            ...
            {
                cachedErrorsMap.put(new TopicPartition(topicResult.name(), partitionResult.partitionIndex()), Errors.forCode(partitionResult.errorCode()));
            }
        }
        return cachedErrorsMap;
    }
