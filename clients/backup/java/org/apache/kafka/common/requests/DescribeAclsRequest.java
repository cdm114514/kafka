...
public class DescribeAclsRequest extends AbstractRequest
{
    ...
    public static class Builder extends AbstractRequest.Builder<DescribeAclsRequest>
    {
        ...
        public Builder(AclBindingFilter filter)
        {
            ...
            data = new DescribeAclsRequestData().setHostFilter(entryFilter.host()).setOperation(entryFilter.operation().code()).setPermissionType(entryFilter.permissionType().code()).setPrincipalFilter(entryFilter.principal()).setResourceNameFilter(patternFilter.name()).setPatternTypeFilter(patternFilter.patternType().code()).setResourceTypeFilter(patternFilter.resourceType().code());
        }
    ...
    private void normalizeAndValidate(short version)
    {
        ...
        if (data.patternTypeFilter() == PatternType.UNKNOWN.code() || data.resourceTypeFilter() == ResourceType.UNKNOWN.code() || data.permissionType() == AclPermissionType.UNKNOWN.code() || data.operation() == AclOperation.UNKNOWN.code())
        {
