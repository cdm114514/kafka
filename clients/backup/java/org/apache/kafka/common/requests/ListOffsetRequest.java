...
public class ListOffsetRequest extends AbstractRequest
{
    ...
    public static class Builder extends AbstractRequest.Builder<ListOffsetRequest>
    {
        ...
        private Builder(short oldestAllowedVersion, short latestAllowedVersion, int replicaId, IsolationLevel isolationLevel)
        {
            ...
            data = new ListOffsetRequestData().setIsolationLevel(isolationLevel.id()).setReplicaId(replicaId);
        }
    ...
    public AbstractResponse getErrorResponse(int throttleTimeMs, Throwable e)
    {
        ...
        for (ListOffsetTopic topic : data.topics())
        {
            ...
            for (ListOffsetPartition partition : topic.partitions())
            {
                ListOffsetPartitionResponse partitionResponse = new ListOffsetPartitionResponse().setErrorCode(errorCode).setPartitionIndex(partition.partitionIndex());
                if (versionId == 0)
                {
                    partitionResponse.setOldStyleOffsets(Collections.emptyList());
                } else
                {
                    partitionResponse.setOffset(ListOffsetResponse.UNKNOWN_OFFSET).setTimestamp(ListOffsetResponse.UNKNOWN_TIMESTAMP);
                }
    ...
    {
        return new ListOffsetTopic().setName(topic).setPartitions(Collections.singletonList(new ListOffsetPartition().setPartitionIndex(partitionIndex).setTimestamp(timestamp).setMaxNumOffsets(maxNumOffsets)));
    }
}
