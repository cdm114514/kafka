...
public class ApiVersionsRequest extends AbstractRequest
{
    ...
    public boolean isValid()
    {
        if (version() >= 3)
        {
            return SOFTWARE_NAME_VERSION_PATTERN.matcher(data.clientSoftwareName()).matches() && SOFTWARE_NAME_VERSION_PATTERN.matcher(data.clientSoftwareVersion()).matches();
        } else
        {
            return true;
        }
    ...
    public ApiVersionsResponse getErrorResponse(int throttleTimeMs, Throwable e)
    {
        ...
        if (Errors.forException(e) == Errors.UNSUPPORTED_VERSION)
        {
            ...
            apiKeys.add(new ApiVersionsResponseKey().setApiKey(ApiKeys.API_VERSIONS.id).setMinVersion(ApiKeys.API_VERSIONS.oldestVersion()).setMaxVersion(ApiKeys.API_VERSIONS.latestVersion()));
