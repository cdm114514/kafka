...
public class ProduceResponse extends AbstractResponse
{
    ...
    private static final Field.Int64 LOG_START_OFFSET_FIELD = new Field.Int64(LOG_START_OFFSET_KEY_NAME, "The start offset of the log at the time this produce response was created", INVALID_OFFSET);
    ...
    private static final Schema PRODUCE_RESPONSE_V0 = new Schema(new Field(RESPONSES_KEY_NAME, new ArrayOf(new Schema(TOPIC_NAME, new Field(PARTITION_RESPONSES_KEY_NAME, new ArrayOf(new Schema(PARTITION_ID, ERROR_CODE, new Field(BASE_OFFSET_KEY_NAME, INT64))))))));

    private static final Schema PRODUCE_RESPONSE_V1 = new Schema(new Field(RESPONSES_KEY_NAME, new ArrayOf(new Schema(TOPIC_NAME, new Field(PARTITION_RESPONSES_KEY_NAME, new ArrayOf(new Schema(PARTITION_ID, ERROR_CODE, new Field(BASE_OFFSET_KEY_NAME, INT64))))))), THROTTLE_TIME_MS);
     ...
     */
    private static final Schema PRODUCE_RESPONSE_V2 = new Schema(new Field(RESPONSES_KEY_NAME, new ArrayOf(new Schema(TOPIC_NAME, new Field(PARTITION_RESPONSES_KEY_NAME, new ArrayOf(new Schema(PARTITION_ID, ERROR_CODE, new Field(BASE_OFFSET_KEY_NAME, INT64), new Field(LOG_APPEND_TIME_KEY_NAME, INT64, "The timestamp returned by broker after appending " + "the messages. If CreateTime is used for the topic, the timestamp will be -1. " + "If LogAppendTime is used for the topic, the timestamp will be " + "the broker local time when the messages are appended."))))))), THROTTLE_TIME_MS);
     ...
     * on the client.
     */
    public static final Schema PRODUCE_RESPONSE_V5 = new Schema(new Field(RESPONSES_KEY_NAME, new ArrayOf(new Schema(TOPIC_NAME, new Field(PARTITION_RESPONSES_KEY_NAME, new ArrayOf(new Schema(PARTITION_ID, ERROR_CODE, new Field(BASE_OFFSET_KEY_NAME, INT64), new Field(LOG_APPEND_TIME_KEY_NAME, INT64, "The timestamp returned by broker after appending " + "the messages. If CreateTime is used for the topic, the timestamp will be -1. " + "If LogAppendTime is used for the topic, the timestamp will be the broker local " + "time when the messages are appended."), LOG_START_OFFSET_FIELD)))))), THROTTLE_TIME_MS);
    ...
    public ProduceResponse(Struct struct)
    {
        ...
        for (Object topicResponse : struct.getArray(RESPONSES_KEY_NAME))
        {
            ...
            for (Object partResponse : topicRespStruct.getArray(PARTITION_RESPONSES_KEY_NAME))
            {
                ...
                long logAppendTime = partRespStruct.hasField(LOG_APPEND_TIME_KEY_NAME) ? partRespStruct.getLong(LOG_APPEND_TIME_KEY_NAME) : RecordBatch.NO_TIMESTAMP;
    ...
    protected Struct toStruct(short version)
    {
        ...
        for (Map.Entry<String, Map<Integer, PartitionResponse>> entry : responseByTopic.entrySet())
        {
            ...
            for (Map.Entry<Integer, PartitionResponse> partitionEntry : entry.getValue().entrySet())
            {
                ...
                Struct partStruct = topicData.instance(PARTITION_RESPONSES_KEY_NAME).set(PARTITION_ID, partitionEntry.getKey()).set(ERROR_CODE, errorCode).set(BASE_OFFSET_KEY_NAME, part.baseOffset);
                ...
                if (partStruct.hasField(RECORD_ERRORS_KEY_NAME))
                {
                    ...
                    if (!part.recordErrors.isEmpty())
                    {
                        ...
                        {
                            Struct indexAndMessageStruct = partStruct.instance(RECORD_ERRORS_KEY_NAME).set(BATCH_INDEX_KEY_NAME, indexAndMessage.batchIndex).set(BATCH_INDEX_ERROR_MESSAGE_FIELD, indexAndMessage.message);
    ...
    public Map<Errors, Integer> errorCounts()
    {
        ...
        responses.values().forEach(response -> updateErrorCounts(errorCounts, response.error));
        return errorCounts;
    }
