...
public class UpdateFeaturesResponse extends AbstractResponse
{
    ...
    {
        return data.results().valuesSet().stream().collect(Collectors.toMap(result -> result.feature(), result -> new ApiError(Errors.forCode(result.errorCode()), result.errorMessage())));
    }
