...
public class Percentiles extends SampledStat implements CompoundStat
{
    ...
    protected void update(Sample sample, MetricConfig config, double value, long timeMs)
    {
        ...
        if (value > max)
        {
            log.warn("Received value {} which is greater than max recordable value {}, will be pinned to the max value", value, max);
            boundedValue = max;
        } else if (value < min)
        {
            log.warn("Received value {} which is less than min recordable value {}, will be pinned to the min value", value, min);
            boundedValue = min;
        } else
        {
            boundedValue = value;
