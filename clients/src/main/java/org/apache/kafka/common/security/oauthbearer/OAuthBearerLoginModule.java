...
public class OAuthBearerLoginModule implements LoginModule
{
    ...
    {
        NOT_LOGGED_IN, LOGGED_IN_NOT_COMMITTED, COMMITTED
    }
    ...
    public boolean login() throws LoginException
    {
        if (loginState == LoginState.LOGGED_IN_NOT_COMMITTED)
        {
            ...
            {
                throw new IllegalStateException(String.format("Already have an uncommitted token with private credential token count=%d", committedTokenCount()));
            } else
            {
        ...
        if (loginState == LoginState.COMMITTED)
        {
            ...
            {
                throw new IllegalStateException(String.format("Already have a committed token with private credential token count=%d; must login on another login context or logout here first before reusing the same login context", committedTokenCount()));
            } else
            {
        ...
        log.debug("Login succeeded; invoke commit() to commit it; current committed token count={}", committedTokenCount());
        return true;
    }
