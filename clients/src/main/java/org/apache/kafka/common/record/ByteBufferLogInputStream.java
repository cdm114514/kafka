...
class ByteBufferLogInputStream implements LogInputStream<MutableRecordBatch>
{
    ...
    Integer nextBatchSize() throws CorruptRecordException
    {
        ...
        {
            throw new CorruptRecordException(String.format("Record size %d is less than the minimum record overhead (%d)", recordSize, LegacyRecord.RECORD_OVERHEAD_V0));
        }
        if (recordSize > maxMessageSize)
        {
            throw new CorruptRecordException(String.format("Record size %d exceeds the largest allowable message size (%d).", recordSize, maxMessageSize));
        }
