...
public class SaslClientAuthenticator implements Authenticator
{
    ...
    private void handleSaslHandshakeResponse(SaslHandshakeResponse response)
    {
        ...
        switch (error)
        {
            ...
            default:
                throw new IllegalSaslStateException(String.format("Unknown error code %s, client mechanism is %s, enabled mechanisms are %s", response.error(), mechanism, response.enabledMechanisms()));
        }
    }
    ...
    private class ReauthInfo
    {
        ...
        public void setAuthenticationEndAndSessionReauthenticationTimes(long nowNanos)
        {
            ...
            if (positiveSessionLifetimeMs != null)
            {
                ...
                log.debug("Finished {} with session expiration in {} ms and session re-authentication on or after {} ms", authenticationOrReauthenticationText(), positiveSessionLifetimeMs, sessionLifetimeMsToUse);
            } else
            {
                log.debug("Finished {} with no session expiration and no session re-authentication", authenticationOrReauthenticationText());
            }
        }

        public Long reauthenticationLatencyMs()
        {
            return reauthenticating() ? Math.round((authenticationEndNanos - reauthenticationBeginNanos) / 1000.0 / 1000.0) : null;
        }
