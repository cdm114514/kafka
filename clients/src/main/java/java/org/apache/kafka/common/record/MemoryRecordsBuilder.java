...
public class MemoryRecordsBuilder implements AutoCloseable
{
    ...
    private int writeDefaultBatchHeader()
    {
        ...
        DefaultRecordBatch.writeHeader(buffer, baseOffset, offsetDelta, size, magic, compressionType, timestampType, firstTimestamp, maxTimestamp, producerId, producerEpoch, baseSequence, isTransactional, isControlBatch, partitionLeaderEpoch, numRecords);
    ...
    public Long appendControlRecordWithOffset(long offset, SimpleRecord record)
    {
        ...
        return appendWithOffset(offset, true, record.timestamp(), record.key(), record.value(), record.headers());
    }
    ...
    public void appendUncheckedWithOffset(long offset, SimpleRecord record) throws IOException
    {
        if (magic >= RecordBatch.MAGIC_VALUE_V2)
        {
            ...
            int sizeInBytes = DefaultRecord.writeTo(appendStream, offsetDelta, timestamp - firstTimestamp, record.key(), record.value(), record.headers());
