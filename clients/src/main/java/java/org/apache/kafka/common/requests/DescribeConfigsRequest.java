...
public class DescribeConfigsRequest extends AbstractRequest
{
    ...
    public DescribeConfigsResponse getErrorResponse(int throttleTimeMs, Throwable e)
    {
        ...
        return new DescribeConfigsResponse(new DescribeConfigsResponseData().setThrottleTimeMs(throttleTimeMs).setResults(data.resources().stream().map(result ->
        {
            return new DescribeConfigsResponseData.DescribeConfigsResult().setErrorCode(error.code()).setErrorMessage(error.message()).setResourceName(result.resourceName()).setResourceType(result.resourceType());
