...
public class IncrementalAlterConfigsResponse extends AbstractResponse
{
    ...
    public static IncrementalAlterConfigsResponseData toResponseData(final int requestThrottleMs, final Map<ConfigResource, ApiError> results)
    {
        ...
        {
            responseData.responses().add(new AlterConfigsResourceResponse().setResourceName(entry.getKey().name()).setResourceType(entry.getKey().type().id()).setErrorCode(entry.getValue().error().code()).setErrorMessage(entry.getValue().message()));
        }
        return responseData;
    }
    ...
    public Map<Errors, Integer> errorCounts()
    {
        ...
        data.responses().forEach(response -> updateErrorCounts(counts, Errors.forCode(response.errorCode())));
        return counts;
    }
