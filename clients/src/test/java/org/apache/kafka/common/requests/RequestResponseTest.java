...
public class RequestResponseTest
{
    ...
    private void verifyDescribeConfigsResponse(DescribeConfigsResponse expected, DescribeConfigsResponse actual, int version) throws Exception
    {
        for (Map.Entry<ConfigResource, DescribeConfigsResult> resource : expected.resultMap().entrySet())
        {
            ...
            for (DescribeConfigsResourceResult actualEntry : actualEntries)
            {
                ...
                assertEquals("Non-matching values for " + actualEntry.name() + " in version " + version, expectedEntry.value(), actualEntry.value());
                assertEquals("Non-matching readonly for " + actualEntry.name() + " in version " + version, expectedEntry.readOnly(), actualEntry.readOnly());
                assertEquals("Non-matching isSensitive for " + actualEntry.name() + " in version " + version, expectedEntry.isSensitive(), actualEntry.isSensitive());
                if (version < 3)
                {
                    assertEquals("Non-matching configType for " + actualEntry.name() + " in version " + version, ConfigType.UNKNOWN.id(), actualEntry.configType());
                } else
                {
                    assertEquals("Non-matching configType for " + actualEntry.name() + " in version " + version, expectedEntry.configType(), actualEntry.configType());
                }
                if (version == 1 || version == 3 || (expectedEntry.configSource() != DescribeConfigsResponse.ConfigSource.DYNAMIC_BROKER_CONFIG.id() && expectedEntry.configSource() != DescribeConfigsResponse.ConfigSource.DYNAMIC_DEFAULT_BROKER_CONFIG.id()))
                {
                    assertEquals("Non-matching configSource for " + actualEntry.name() + " in version " + version, expectedEntry.configSource(), actualEntry.configSource());
                } else
                {
                    assertEquals("Non matching configSource for " + actualEntry.name() + " in version " + version, DescribeConfigsResponse.ConfigSource.STATIC_BROKER_CONFIG.id(), actualEntry.configSource());
                }
            }
        }
    }
    ...
    private void checkDescribeConfigsResponseVersions() throws Exception
    {
        ...
        DescribeConfigsResponse deserialized0 = (DescribeConfigsResponse) deserialize(response, response.toStruct((short) 0), (short) 0);
        verifyDescribeConfigsResponse(response, deserialized0, 0);

        response = createDescribeConfigsResponse((short) 1);
        DescribeConfigsResponse deserialized1 = (DescribeConfigsResponse) deserialize(response, response.toStruct((short) 1), (short) 1);
        verifyDescribeConfigsResponse(response, deserialized1, 1);

        response = createDescribeConfigsResponse((short) 3);
        DescribeConfigsResponse deserialized3 = (DescribeConfigsResponse) deserialize(response, response.toStruct((short) 3), (short) 3);
    ...
    private void checkErrorResponse(AbstractRequest req, Throwable e, boolean checkEqualityAndHashCode)
    {
        ...
        if (e instanceof UnknownServerException)
        {
            ...
            assertFalse(String.format("Unknown message included in response for %s: %s ", req.api, responseStr), responseStr.contains(e.getMessage()));
        }
    }
    ...
    public void fetchResponseVersionTest()
    {
        ...
        responseData.put(new TopicPartition("test", 0), new FetchResponse.PartitionData<>(Errors.NONE, 1000000, FetchResponse.INVALID_LAST_STABLE_OFFSET, 0L, Optional.empty(), Collections.emptyList(), records));
        ...
        assertEquals("Should use schema version 0", FETCH.responseSchema((short) 0), v0Response.toStruct((short) 0).schema());
        assertEquals("Should use schema version 1", FETCH.responseSchema((short) 1), v1Response.toStruct((short) 1).schema());
    ...
    public void testFetchResponseV4()
    {
        ...
        responseData.put(new TopicPartition("bar", 0), new FetchResponse.PartitionData<>(Errors.NONE, 100000, FetchResponse.INVALID_LAST_STABLE_OFFSET, FetchResponse.INVALID_LOG_START_OFFSET, Optional.empty(), abortedTransactions, records));
        responseData.put(new TopicPartition("bar", 1), new FetchResponse.PartitionData<>(Errors.NONE, 900000, 5, FetchResponse.INVALID_LOG_START_OFFSET, Optional.empty(), null, records));
        responseData.put(new TopicPartition("foo", 0), new FetchResponse.PartitionData<>(Errors.NONE, 70000, 6, FetchResponse.INVALID_LOG_START_OFFSET, Optional.empty(), emptyList(), records));
    ...
    public void verifyFetchResponseFullWrites() throws Exception
    {
        ...
        verifyFetchResponseFullWrite(FETCH.latestVersion(), createFetchResponse(Errors.FETCH_SESSION_ID_NOT_FOUND, 123));
    ...
    public void testFetchRequestCompat()
    {
        ...
        FetchRequest req = FetchRequest.Builder.forConsumer(100, 100, fetchData).metadata(new FetchMetadata(10, 20)).isolationLevel(IsolationLevel.READ_COMMITTED).build((short) 2);
    ...
    public void testOffsetFetchRequestBuilderToString()
    {
        ...
        for (Boolean requireStable : stableFlags)
        {
            ...
            assertTrue(allTopicPartitionsString.contains("groupId='someGroup', topics=null, requireStable=" + requireStable.toString()));
            String string = new OffsetFetchRequest.Builder("group1", requireStable, Collections.singletonList(new TopicPartition("test11", 1)), false).toString();
    ...
    {
        ListGroupsRequestData data = new ListGroupsRequestData().setStatesFilter(asList(ConsumerGroupState.STABLE.name()));
    ...
    public void testApiVersionResponseStructParsingFallback()
    {
        ...
        ApiVersionsResponse response = ApiVersionsResponse.fromStruct(struct, ApiKeys.API_VERSIONS.latestVersion());
    ...
    {
        InitProducerIdRequest.Builder bld = new InitProducerIdRequest.Builder(new InitProducerIdRequestData().setTransactionTimeoutMs(1000).setTransactionalId("abracadabra").setProducerId(123));
        final UnsupportedVersionException exception = assertThrows(UnsupportedVersionException.class, () -> bld.build((short) 2).toStruct());
    ...
    {
        DeletableTopicResult result = new DeletableTopicResult().setName("topic").setErrorCode(Errors.THROTTLING_QUOTA_EXCEEDED.code());
    ...
    private FetchRequest createFetchRequest(int version, FetchMetadata metadata, List<TopicPartition> toForget)
    {
        ...
        fetchData.put(new TopicPartition("test1", 0), new FetchRequest.PartitionData(100, -1L, 1000000, Optional.empty()));
        fetchData.put(new TopicPartition("test2", 0), new FetchRequest.PartitionData(200, -1L, 1000000, Optional.empty()));
        return FetchRequest.Builder.forConsumer(100, 100000, fetchData).metadata(metadata).setMaxBytes(1000).toForget(toForget).build((short) version);
    }
    ...
    private FetchRequest createFetchRequest(int version, IsolationLevel isolationLevel)
    {
        ...
        fetchData.put(new TopicPartition("test1", 0), new FetchRequest.PartitionData(100, -1L, 1000000, Optional.empty()));
        fetchData.put(new TopicPartition("test2", 0), new FetchRequest.PartitionData(200, -1L, 1000000, Optional.empty()));
        return FetchRequest.Builder.forConsumer(100, 100000, fetchData).isolationLevel(isolationLevel).setMaxBytes(1000).build((short) version);
    }
    ...
    private FetchResponse<MemoryRecords> createFetchResponse(boolean includeAborted)
    {
        ...
        if (includeAborted)
        {
            abortedTransactions = Collections.singletonList(new FetchResponse.AbortedTransaction(234L, 999L));
        }
        responseData.put(new TopicPartition("test", 1), new FetchResponse.PartitionData<>(Errors.NONE, 1000000, FetchResponse.INVALID_LAST_STABLE_OFFSET, 0L, Optional.empty(), abortedTransactions, MemoryRecords.EMPTY));
    ...
    private JoinGroupResponse createJoinGroupResponse(int version)
    {
        ...
        JoinGroupResponseData data = new JoinGroupResponseData().setErrorCode(Errors.NONE.code()).setGenerationId(1).setProtocolType("consumer") // Added in v7 but ignorable
                .setProtocolName("range").setLeader("leader").setMemberId("consumer1").setMembers(members);
    ...
    {
        List<SyncGroupRequestAssignment> assignments = Collections.singletonList(new SyncGroupRequestAssignment().setMemberId("member").setAssignment(new byte[0]));

        SyncGroupRequestData data = new SyncGroupRequestData().setGroupId("group1").setGenerationId(1).setMemberId("member").setProtocolType("consumer") // Added in v5 but ignorable
    ...
    {
        SyncGroupResponseData data = new SyncGroupResponseData().setErrorCode(Errors.NONE.code()).setProtocolType("consumer") // Added in v5 but ignorable
                .setProtocolName("range")    // Added in v5 but ignorable
    ...
    private DescribeGroupsResponse createDescribeGroupResponse()
    {
        ...
        DescribeGroupsResponseData.DescribedGroupMember member = DescribeGroupsResponse.groupMember("memberId", null, clientId, clientHost, new byte[0], new byte[0]);
        DescribedGroup metadata = DescribeGroupsResponse.groupMetadata("test-group", Errors.NONE, "STABLE", "consumer", "roundrobin", Collections.singletonList(member), DescribeGroupsResponse.AUTHORIZED_OPERATIONS_OMITTED);
    ...
    {
        return new LeaveGroupRequest.Builder("group1", Collections.singletonList(new MemberIdentity().setMemberId("consumer1"))).build();
    }
    ...
    private DeleteGroupsResponse createDeleteGroupsResponse()
    {
        ...
        result.add(new DeletableGroupResult().setGroupId("test-group").setErrorCode(Errors.NONE.code()));
    ...
    private ListOffsetRequest createListOffsetRequest(int version)
    {
        if (version == 0)
        {
            ListOffsetTopic topic = new ListOffsetTopic().setName("test").setPartitions(Arrays.asList(new ListOffsetPartition().setPartitionIndex(0).setTimestamp(1000000L).setMaxNumOffsets(10).setCurrentLeaderEpoch(5)));
            return ListOffsetRequest.Builder.forConsumer(false, IsolationLevel.READ_UNCOMMITTED).setTargetTimes(Collections.singletonList(topic)).build((short) version);
        } else if (version == 1)
        {
            ListOffsetTopic topic = new ListOffsetTopic().setName("test").setPartitions(Arrays.asList(new ListOffsetPartition().setPartitionIndex(0).setTimestamp(1000000L).setCurrentLeaderEpoch(5)));
            return ListOffsetRequest.Builder.forConsumer(true, IsolationLevel.READ_UNCOMMITTED).setTargetTimes(Collections.singletonList(topic)).build((short) version);
        } else if (version >= 2 && version <= 5)
        {
            ListOffsetPartition partition = new ListOffsetPartition().setPartitionIndex(0).setTimestamp(1000000L).setCurrentLeaderEpoch(5);

            ListOffsetTopic topic = new ListOffsetTopic().setName("test").setPartitions(Arrays.asList(partition));
            return ListOffsetRequest.Builder.forConsumer(true, IsolationLevel.READ_COMMITTED).setTargetTimes(Collections.singletonList(topic)).build((short) version);
        } else
        {
    ...
    private ListOffsetResponse createListOffsetResponse(int version)
    {
        if (version == 0)
        {
            ListOffsetResponseData data = new ListOffsetResponseData().setTopics(Collections.singletonList(new ListOffsetTopicResponse().setName("test").setPartitions(Collections.singletonList(new ListOffsetPartitionResponse().setPartitionIndex(0).setErrorCode(Errors.NONE.code()).setOldStyleOffsets(asList(100L))))));
            return new ListOffsetResponse(data);
        } else if (version >= 1 && version <= 5)
        {
            ListOffsetPartitionResponse partition = new ListOffsetPartitionResponse().setPartitionIndex(0).setErrorCode(Errors.NONE.code()).setTimestamp(10000L).setOffset(100L);
            if (version >= 4)
            {
                partition.setLeaderEpoch(27);
            }
            ListOffsetResponseData data = new ListOffsetResponseData().setTopics(Collections.singletonList(new ListOffsetTopicResponse().setName("test").setPartitions(Collections.singletonList(partition))));
            return new ListOffsetResponse(data);
        } else
        {
    ...
    private MetadataResponse createMetadataResponse()
    {
        ...
        allTopicMetadata.add(new MetadataResponse.TopicMetadata(Errors.NONE, "__consumer_offsets", true, asList(new MetadataResponse.PartitionMetadata(Errors.NONE, new TopicPartition("__consumer_offsets", 1), Optional.of(node.id()), Optional.of(5), replicas, isr, offlineReplicas))));
        allTopicMetadata.add(new MetadataResponse.TopicMetadata(Errors.LEADER_NOT_AVAILABLE, "topic2", false, emptyList()));
        allTopicMetadata.add(new MetadataResponse.TopicMetadata(Errors.NONE, "topic3", false, asList(new MetadataResponse.PartitionMetadata(Errors.LEADER_NOT_AVAILABLE, new TopicPartition("topic3", 0), Optional.empty(), Optional.empty(), replicas, isr, offlineReplicas))));
    ...
    {
        return new OffsetFetchRequest.Builder("group1", requireStable, Collections.singletonList(new TopicPartition("test11", 1)), false).build((short) version);
    }
    ...
    {
        Map<TopicPartition, ProduceResponse.PartitionResponse> responseData = new HashMap<>();
        responseData.put(new TopicPartition("test", 0), new ProduceResponse.PartitionResponse(Errors.NONE, 10000, RecordBatch.NO_TIMESTAMP, 100, Collections.singletonList(new ProduceResponse.RecordError(0, "error message")), "global error message"));
        return new ProduceResponse(responseData, 0);
    }
    ...
    private StopReplicaRequest createStopReplicaRequest(int version, boolean deletePartitions)
    {
        ...
        StopReplicaTopicState topic1 = new StopReplicaTopicState().setTopicName("topic1").setPartitionStates(Collections.singletonList(new StopReplicaPartitionState().setPartitionIndex(0).setLeaderEpoch(1).setDeletePartition(deletePartitions)));
        topicStates.add(topic1);
        StopReplicaTopicState topic2 = new StopReplicaTopicState().setTopicName("topic2").setPartitionStates(Collections.singletonList(new StopReplicaPartitionState().setPartitionIndex(1).setLeaderEpoch(2).setDeletePartition(deletePartitions)));
        topicStates.add(topic2);

        return new StopReplicaRequest.Builder((short) version, 0, 1, 0, deletePartitions, topicStates).build((short) version);
    }
    ...
    private ControlledShutdownRequest createControlledShutdownRequest()
    {
        ...
        return new ControlledShutdownRequest.Builder(data, ApiKeys.CONTROLLED_SHUTDOWN.latestVersion()).build();
    }
    ...
    {
        RemainingPartition p1 = new RemainingPartition().setTopicName("test2").setPartitionIndex(5);
        RemainingPartition p2 = new RemainingPartition().setTopicName("test1").setPartitionIndex(10);
    ...
    private UpdateMetadataRequest createUpdateMetadataRequest(int version, String rack)
    {
        ...
        List<UpdateMetadataBroker> liveBrokers = Arrays.asList(new UpdateMetadataBroker().setId(0).setEndpoints(endpoints1).setRack(rack), new UpdateMetadataBroker().setId(1).setEndpoints(endpoints2).setRack(rack));
        return new UpdateMetadataRequest.Builder((short) version, 1, 10, 0, partitionStates, liveBrokers).build();
    }
    ...
    private ApiVersionsResponse createApiVersionResponse()
    {
        ...
        apiVersions.add(new ApiVersionsResponseKey().setApiKey((short) 0).setMinVersion((short) 0).setMaxVersion((short) 2));
    ...
    private CreateTopicsRequest createCreateTopicRequest(int version, boolean validateOnly)
    {
        ...
        topic2.assignments().add(new CreatableReplicaAssignment().setPartitionIndex(0).setBrokerIds(Arrays.asList(1, 2, 3)));
        topic2.assignments().add(new CreatableReplicaAssignment().setPartitionIndex(1).setBrokerIds(Arrays.asList(2, 3, 4)));
    ...
    private CreateTopicsResponse createCreateTopicResponse()
    {
        ...
        data.topics().add(new CreatableTopicResult().setName("t3").setErrorCode(Errors.NONE.code()).setNumPartitions(1).setReplicationFactor((short) 2).setConfigs(Collections.singletonList(new CreatableTopicConfigs().setName("min.insync.replicas").setValue("2"))));
        return new CreateTopicsResponse(data);
    }

    private DeleteTopicsRequest createDeleteTopicsRequest(int version)
    {
        return new DeleteTopicsRequest.Builder(new DeleteTopicsRequestData().setTopicNames(Arrays.asList("my_t1", "my_t2")).setTimeoutMs(1000)).build((short) version);
    }
    ...
    {
        InitProducerIdResponseData responseData = new InitProducerIdResponseData().setErrorCode(Errors.NONE.code()).setProducerEpoch((short) 3).setProducerId(3332).setThrottleTimeMs(0);
    ...
    private TxnOffsetCommitRequest createTxnOffsetCommitRequest(int version)
    {
        ...
        offsets.put(new TopicPartition("topic", 73), new TxnOffsetCommitRequest.CommittedOffset(100, null, Optional.empty()));
        offsets.put(new TopicPartition("topic", 74), new TxnOffsetCommitRequest.CommittedOffset(100, "blah", Optional.of(27)));

        if (version < 3)
        {
            return new TxnOffsetCommitRequest.Builder("transactionalId", "groupId", 21L, (short) 42, offsets, false).build();
        } else
        {
            return new TxnOffsetCommitRequest.Builder("transactionalId", "groupId", 21L, (short) 42, offsets, "member", 2, Optional.of("instance"), false).build();
        }
    }
    ...
    private TxnOffsetCommitRequest createTxnOffsetCommitRequestWithAutoDowngrade(int version)
    {
        ...
        return new TxnOffsetCommitRequest.Builder("transactionalId", "groupId", 21L, (short) 42, offsets, "member", 2, Optional.of("instance"), true).build();
    }
    ...
    {
        return new DescribeAclsRequest.Builder(new AclBindingFilter(new ResourcePatternFilter(ResourceType.TOPIC, "mytopic", PatternType.LITERAL), new AccessControlEntryFilter(null, null, AclOperation.ANY, AclPermissionType.ANY))).build();
    }
    ...
    {
        return new DescribeConfigsRequest.Builder(new DescribeConfigsRequestData().setResources(asList(new DescribeConfigsRequestData.DescribeConfigsResource().setResourceType(ConfigResource.Type.BROKER.id()).setResourceName("0"), new DescribeConfigsRequestData.DescribeConfigsResource().setResourceType(ConfigResource.Type.TOPIC.id()).setResourceName("topic")))).build((short) version);
    }

    private DescribeConfigsRequest createDescribeConfigsRequestWithConfigEntries(int version)
    {
        return new DescribeConfigsRequest.Builder(new DescribeConfigsRequestData().setResources(asList(new DescribeConfigsRequestData.DescribeConfigsResource().setResourceType(ConfigResource.Type.BROKER.id()).setResourceName("0").setConfigurationKeys(asList("foo", "bar")), new DescribeConfigsRequestData.DescribeConfigsResource().setResourceType(ConfigResource.Type.TOPIC.id()).setResourceName("topic").setConfigurationKeys(null), new DescribeConfigsRequestData.DescribeConfigsResource().setResourceType(ConfigResource.Type.TOPIC.id()).setResourceName("topic a").setConfigurationKeys(emptyList())))).build((short) version);
    }

    private DescribeConfigsRequest createDescribeConfigsRequestWithDocumentation(int version)
    {
        DescribeConfigsRequestData data = new DescribeConfigsRequestData().setResources(asList(new DescribeConfigsRequestData.DescribeConfigsResource().setResourceType(ConfigResource.Type.BROKER.id()).setResourceName("0").setConfigurationKeys(asList("foo", "bar"))));
        if (version == 3)
        {
    ...
    {
        return new DescribeConfigsResponse(new DescribeConfigsResponseData().setResults(asList(new DescribeConfigsResult().setErrorCode(Errors.NONE.code()).setResourceType(ConfigResource.Type.BROKER.id()).setResourceName("0").setConfigs(asList(new DescribeConfigsResourceResult().setName("config_name").setValue("config_value")
    ...
    private AlterConfigsRequest createAlterConfigsRequest()
    {
        ...
        configs.put(new ConfigResource(ConfigResource.Type.TOPIC, "topic"), new AlterConfigsRequest.Config(Collections.<AlterConfigsRequest.ConfigEntry>emptyList()));
    ...
    {
        CreatePartitionsTopicCollection topics = new CreatePartitionsTopicCollection();
        CreatePartitionsAssignment myTopicAssignment = new CreatePartitionsAssignment().setBrokerIds(Collections.singletonList(2));
        topics.add(new CreatePartitionsTopic().setName("my_topic").setCount(3).setAssignments(Collections.singletonList(myTopicAssignment)));
    ...
    private CreateDelegationTokenRequest createCreateTokenRequest()
    {
        ...
        return new CreateDelegationTokenRequest.Builder(new CreateDelegationTokenRequestData().setRenewers(renewers).setMaxLifetimeMs(System.currentTimeMillis())).build();
    }
    ...
    private IncrementalAlterConfigsResponse createIncrementalAlterConfigsResponse()
    {
        ...
        data.responses().add(new AlterConfigsResourceResponse().setResourceName("testtopic").setResourceType(ResourceType.TOPIC.code()).setErrorCode(Errors.INVALID_REQUEST.code()).setErrorMessage("Duplicate Keys"));
    ...
    private OffsetDeleteRequest createOffsetDeleteRequest()
    {
        ...
        topics.add(new OffsetDeleteRequestTopic().setName("topic1").setPartitions(Collections.singletonList(new OffsetDeleteRequestPartition().setPartitionIndex(0))));
    ...
    private AlterReplicaLogDirsRequest createAlterReplicaLogDirsRequest()
    {
        ...
        data.dirs().add(new AlterReplicaLogDirsRequestData.AlterReplicaLogDir().setPath("/data0").setTopics(new AlterReplicaLogDirTopicCollection(Collections.singletonList(new AlterReplicaLogDirTopic().setPartitions(singletonList(0)).setName("topic")).iterator())));
    ...
    private AlterReplicaLogDirsResponse createAlterReplicaLogDirsResponse()
    {
        ...
        data.results().add(new AlterReplicaLogDirsResponseData.AlterReplicaLogDirTopicResult().setTopicName("topic").setPartitions(Collections.singletonList(new AlterReplicaLogDirsResponseData.AlterReplicaLogDirPartitionResult().setPartitionIndex(0).setErrorCode(Errors.LOG_DIR_NOT_FOUND.code()))));
