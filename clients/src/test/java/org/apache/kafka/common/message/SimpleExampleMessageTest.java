...
public class SimpleExampleMessageTest
{
    ...
    public void shouldThrowIfCannotWriteNonIgnorableField()
    {
        ...
        assertThrows(UnsupportedVersionException.class, () -> out.write(new ByteBufferAccessor(ByteBuffer.allocate(64)), new ObjectSerializationCache(), (short) 0));
    ...
    public void testMyTaggedIntArray()
    {
        ...
        testRoundTrip(new SimpleExampleMessageData(), message -> assertEquals(Collections.emptyList(), message.myTaggedIntArray()));

        // Verify that we can set a tagged array of ints.
        testRoundTrip(new SimpleExampleMessageData().setMyTaggedIntArray(Arrays.asList(1, 2, 3)), message -> assertEquals(Arrays.asList(1, 2, 3), message.myTaggedIntArray()));
    }
    ...
    public void testMyNullableString()
    {
        ...
        testRoundTrip(new SimpleExampleMessageData(), message -> assertTrue(message.myNullableString() == null));

        // Verify that we can set and retrieve a string for the tagged field.
        testRoundTrip(new SimpleExampleMessageData().setMyNullableString("foobar"), message -> assertEquals("foobar", message.myNullableString()));
    }
    ...
    public void testMyInt16()
    {
        ...
        testRoundTrip(new SimpleExampleMessageData(), message -> assertEquals((short) 123, message.myInt16()));

        testRoundTrip(new SimpleExampleMessageData().setMyInt16((short) 456), message -> assertEquals((short) 456, message.myInt16()));
    }
    ...
    public void testMyString()
    {
        ...
        testRoundTrip(new SimpleExampleMessageData(), message -> assertEquals("", message.myString()));

        testRoundTrip(new SimpleExampleMessageData().setMyString("abc"), message -> assertEquals("abc", message.myString()));
    }
    ...
    {
        // Verify that the tagged field reads as empty when not set.
        testRoundTrip(new SimpleExampleMessageData(), message -> assertArrayEquals(new byte[0], message.myBytes()));

        testRoundTrip(new SimpleExampleMessageData().setMyBytes(new byte[]{0x43, 0x66}), message -> assertArrayEquals(new byte[]{0x43, 0x66}, message.myBytes()));

        testRoundTrip(new SimpleExampleMessageData().setMyBytes(null), message -> assertTrue(message.myBytes() == null));
    }

    @Test
    public void testTaggedUuid()
    {
        testRoundTrip(new SimpleExampleMessageData(), message -> assertEquals(UUID.fromString("212d5494-4a8b-4fdf-94b3-88b470beb367"), message.taggedUuid()));

        testRoundTrip(new SimpleExampleMessageData().setTaggedUuid(UUID.fromString("01234567-89ab-cdef-0123-456789abcdef")), message -> assertEquals(UUID.fromString("01234567-89ab-cdef-0123-456789abcdef"), message.taggedUuid()));
    }

    @Test
    public void testTaggedLong()
    {
        testRoundTrip(new SimpleExampleMessageData(), message -> assertEquals(0xcafcacafcacafcaL, message.taggedLong()));

        testRoundTrip(new SimpleExampleMessageData().setMyString("blah").setMyTaggedIntArray(Arrays.asList(4)).setTaggedLong(0x123443211234432L), message -> assertEquals(0x123443211234432L, message.taggedLong()));
    }
    ...
    public void testMyStruct()
    {
        ...
        testRoundTrip(new SimpleExampleMessageData().setMyStruct(myStruct), message -> assertEquals(myStruct, message.myStruct()), (short) 2);
    }
    ...
    public void testMyTaggedStruct()
    {
        ...
        testRoundTrip(new SimpleExampleMessageData().setMyTaggedStruct(myStruct), message -> assertEquals(myStruct, message.myTaggedStruct()), (short) 2);

        // Not setting field works for both version 1 and version 2 protocol
        testRoundTrip(new SimpleExampleMessageData().setMyString("abc"), message -> assertEquals("abc", message.myString()), (short) 1);
        testRoundTrip(new SimpleExampleMessageData().setMyString("abc"), message -> assertEquals("abc", message.myString()), (short) 2);
    }
