...
public class MetricsTest
{
    ...
    private void verifyStats(Function<KafkaMetric, Double> metricValueFunc)
    {
        ...
        assertEquals(String.format("Occurrences(0...%d) = %f", count, count / elapsedSecs), count / elapsedSecs, metricValueFunc.apply(metrics.metrics().get(metrics.metricName("test.occurences", "grp1"))), EPS);
        ...
        assertEquals("Rate(0...9) = 1.40625", sum / elapsedSecs, metricValueFunc.apply(metrics.metrics().get(metrics.metricName("test.rate", "grp1"))), EPS);
        assertEquals(String.format("Occurrences(0...%d) = %f", count, count / elapsedSecs), count / elapsedSecs, metricValueFunc.apply(metrics.metrics().get(metrics.metricName("test.occurences", "grp1"))), EPS);
        assertEquals("Count(0...9) = 10", (double) count, metricValueFunc.apply(metrics.metrics().get(metrics.metricName("test.count", "grp1"))), EPS);
    }
    ...
    enum StatType
    {
        AVG(0), TOTAL(1), COUNT(2), MAX(3), MIN(4), RATE(5), SIMPLE_RATE(6), SUM(7), VALUE(8), PERCENTILES(9), METER(10);
