            ...
            for (boolean required : new boolean[]{true, false})
            {
                for (int allowableClockSkewMs : new int[]{0, 5, 10, 20})
                {
                    for (long whenCheckOffsetMs : new long[]{-10, 0, 10})
                    {
                        ...
                        if (required && !exists)
                        {
                            assertTrue("useErrorValue || required && !exists", isFailureWithMessageAndNoFailureScope(result));
                        ...
                        {
                            assertTrue(assertionFailureMessage(nowClaimValue, allowableClockSkewMs, whenCheckMs), isFailureWithMessageAndNoFailureScope(result));
                        } else
                        {
                            assertTrue(assertionFailureMessage(nowClaimValue, allowableClockSkewMs, whenCheckMs), isSuccess(result));
                        }
                    }
                }
            }
