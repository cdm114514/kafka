...
public class FileRecordsTest
{
    ...
    public void testRead() throws IOException
    {
        ...
        assertEquals("Read a single message starting from the second message", Collections.singletonList(second), batches(read));
    }
    ...
    public void testSearch() throws IOException
    {
        ...
        assertEquals("Should be able to find fourth message from a non-existent offset", new FileRecords.LogOffsetPosition(50L, position, message4Size), fileRecords.searchForOffsetWithSize(3, position));
    ...
    private void testSearchForTimestamp(RecordVersion version) throws IOException
    {
        ...
        assertFoundTimestamp(new FileRecords.TimestampAndOffset(10L, 5, Optional.of(0)), fileRecords.searchForTimestamp(9L, 0, 0L), version);
        assertFoundTimestamp(new FileRecords.TimestampAndOffset(10L, 5, Optional.of(0)), fileRecords.searchForTimestamp(10L, 0, 0L), version);
        assertFoundTimestamp(new FileRecords.TimestampAndOffset(11L, 6, Optional.of(1)), fileRecords.searchForTimestamp(11L, 0, 0L), version);
        assertNull(fileRecords.searchForTimestamp(12L, 0, 0L));
    }

    private void assertFoundTimestamp(FileRecords.TimestampAndOffset expected, FileRecords.TimestampAndOffset actual, RecordVersion version)
    {
        ...
        } else
        {
            ...
            Optional<Integer> expectedLeaderEpoch = version.value >= RecordVersion.V2.value ? expected.leaderEpoch : Optional.empty();
    ...
    private void appendWithOffsetAndTimestamp(FileRecords fileRecords, RecordVersion recordVersion, long timestamp, long offset, int leaderEpoch) throws IOException
    {
        ...
        MemoryRecordsBuilder builder = MemoryRecords.builder(buffer, recordVersion.value, CompressionType.NONE, TimestampType.CREATE_TIME, offset, timestamp, leaderEpoch);
    ...
    public void testDownconversionAfterMessageFormatDowngrade() throws IOException
    {
        ...
        List<SimpleRecord> records = asList(new SimpleRecord(1L, "k1".getBytes(), bytes), new SimpleRecord(2L, "k2".getBytes(), bytes));
        byte toMagic = 1;
    ...
    private void downConvertAndVerifyRecords(List<SimpleRecord> initialRecords, List<Long> initialOffsets, FileRecords fileRecords, CompressionType compressionType, byte toMagic, long firstOffset, Time time)
    {
        ...
        List<Long> maximumReadSize = asList(16L * 1024L, (long) fileRecords.sizeInBytes(), (long) fileRecords.sizeInBytes() - 1, (long) fileRecords.sizeInBytes() / 4, maxBatchSize + 1, 1L);
    ...
    private void append(FileRecords fileRecords, byte[][] values) throws IOException
    {
        ...
        {
            ByteBuffer buffer = ByteBuffer.allocate(128);
            MemoryRecordsBuilder builder = MemoryRecords.builder(buffer, RecordBatch.CURRENT_MAGIC_VALUE, CompressionType.NONE, TimestampType.CREATE_TIME, offset);
