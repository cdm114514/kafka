...
public class ListOffsetRequestTest
{
    ...
    {
        List<ListOffsetTopic> topics = Collections.singletonList(new ListOffsetTopic().setName("topic").setPartitions(Arrays.asList(new ListOffsetPartition().setPartitionIndex(0), new ListOffsetPartition().setPartitionIndex(0))));
    ...
    public void testGetErrorResponse()
    {
        ...
        {
            List<ListOffsetTopic> topics = Arrays.asList(new ListOffsetTopic().setName("topic").setPartitions(Collections.singletonList(new ListOffsetPartition().setPartitionIndex(0))));
            ListOffsetRequest request = ListOffsetRequest.Builder.forConsumer(true, IsolationLevel.READ_COMMITTED).setTargetTimes(topics).build(version);
            ListOffsetResponse response = (ListOffsetResponse) request.getErrorResponse(0, Errors.NOT_LEADER_OR_FOLLOWER.exception());

            List<ListOffsetTopicResponse> v = Collections.singletonList(new ListOffsetTopicResponse().setName("topic").setPartitions(Collections.singletonList(new ListOffsetPartitionResponse().setErrorCode(Errors.NOT_LEADER_OR_FOLLOWER.code()).setLeaderEpoch(ListOffsetResponse.UNKNOWN_EPOCH).setOffset(ListOffsetResponse.UNKNOWN_OFFSET).setPartitionIndex(0).setTimestamp(ListOffsetResponse.UNKNOWN_TIMESTAMP))));
    ...
    {
        List<ListOffsetTopic> topics = Arrays.asList(new ListOffsetTopic().setName("topic").setPartitions(Collections.singletonList(new ListOffsetPartition().setPartitionIndex(0))));
        ListOffsetRequest request = ListOffsetRequest.Builder.forConsumer(true, IsolationLevel.READ_UNCOMMITTED).setTargetTimes(topics).build((short) 0);
        ListOffsetResponse response = (ListOffsetResponse) request.getErrorResponse(0, Errors.NOT_LEADER_OR_FOLLOWER.exception());

        List<ListOffsetTopicResponse> v = Collections.singletonList(new ListOffsetTopicResponse().setName("topic").setPartitions(Collections.singletonList(new ListOffsetPartitionResponse().setErrorCode(Errors.NOT_LEADER_OR_FOLLOWER.code()).setOldStyleOffsets(Collections.emptyList()).setPartitionIndex(0))));
        ListOffsetResponseData data = new ListOffsetResponseData().setThrottleTimeMs(0).setTopics(v);
        ListOffsetResponse expectedResponse = new ListOffsetResponse(data);
        assertEquals(expectedResponse.data().topics(), response.data().topics());
        assertEquals(expectedResponse.throttleTimeMs(), response.throttleTimeMs());
    ...
    {
        ListOffsetPartition lop0 = new ListOffsetPartition().setPartitionIndex(0).setCurrentLeaderEpoch(1).setMaxNumOffsets(2).setTimestamp(123L);
        ListOffsetPartition lop1 = new ListOffsetPartition().setPartitionIndex(1).setCurrentLeaderEpoch(3).setMaxNumOffsets(4).setTimestamp(567L);
