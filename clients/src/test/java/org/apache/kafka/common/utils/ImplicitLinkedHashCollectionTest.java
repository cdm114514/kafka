...
public class ImplicitLinkedHashCollectionTest
{
    ...
    static void expectTraversal(Iterator<TestElement> iterator, Integer... sequence)
    {
        ...
        while (iterator.hasNext())
        {
            ...
            Assert.assertTrue("Iterator yieled " + (i + 1) + " elements, but only " + sequence.length + " were expected.", i < sequence.length);
            Assert.assertEquals("Iterator value number " + (i + 1) + " was incorrect.", sequence[i].intValue(), element.key);
            i = i + 1;
        }
        Assert.assertTrue("Iterator yieled " + (i + 1) + " elements, but " + sequence.length + " were expected.", i == sequence.length);
    }
    ...
    static void expectTraversal(Iterator<TestElement> iter, Iterator<Integer> expectedIter)
    {
        ...
        while (iter.hasNext())
        {
            ...
            Assert.assertEquals("Iterator value number " + (i + 1) + " was incorrect.", expected.intValue(), element.key);
            i = i + 1;
        }
    ...
    public void testMoveToEnd()
    {
        ...
        Assert.assertThrows(RuntimeException.class, () -> coll.moveToEnd(new TestElement(4, 4)));
    }
