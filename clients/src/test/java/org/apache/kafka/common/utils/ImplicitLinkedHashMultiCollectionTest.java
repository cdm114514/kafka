...
public class ImplicitLinkedHashMultiCollectionTest
{
    ...
    static void expectExactTraversal(Iterator<TestElement> iterator, TestElement... sequence)
    {
        ...
        while (iterator.hasNext())
        {
            ...
            assertTrue("Iterator yieled " + (i + 1) + " elements, but only " + sequence.length + " were expected.", i < sequence.length);
        ...
        assertTrue("Iterator yieled " + (i + 1) + " elements, but " + sequence.length + " were expected.", i == sequence.length);
    }
    ...
    public void testEnlargement()
    {
        ...
        TestElement[] testElements = {new TestElement(100), new TestElement(101), new TestElement(102), new TestElement(100), new TestElement(101), new TestElement(105)};
        ...
        expectExactTraversal(multiSet.iterator(), testElements[0], testElements[2], testElements[3], testElements[4], testElements[5]);
    }
    ...
    void expectTraversal(Iterator<TestElement> iter, Iterator<TestElement> expectedIter)
    {
        ...
        while (iter.hasNext())
        {
            ...
            Assert.assertTrue("Iterator yieled " + (i + 1) + " elements, but only " + i + " were expected.", expectedIter.hasNext());
            TestElement expected = expectedIter.next();
            assertTrue("Iterator value number " + (i + 1) + " was incorrect.", expected == element);
            i = i + 1;
        }
        Assert.assertFalse("Iterator yieled " + i + " elements, but at least " + (i + 1) + " were expected.", expectedIter.hasNext());
    }
}
