...
public final class MessageTest
{
    ...
    {
        List<ListOffsetTopic> v = Collections.singletonList(new ListOffsetTopic().setName("topic").setPartitions(Collections.singletonList(new ListOffsetPartition().setPartitionIndex(0).setTimestamp(123L))));
        Supplier<ListOffsetRequestData> newRequest = () -> new ListOffsetRequestData().setTopics(v).setReplicaId(0);
        testAllMessageRoundTrips(newRequest.get());
    ...
    {
        ListOffsetPartitionResponse partition = new ListOffsetPartitionResponse().setErrorCode(Errors.NONE.code()).setPartitionIndex(0).setOldStyleOffsets(Collections.singletonList(321L));
        List<ListOffsetTopicResponse> topics = Collections.singletonList(new ListOffsetTopicResponse().setName("topic").setPartitions(Collections.singletonList(partition)));
        Supplier<ListOffsetResponseData> response = () -> new ListOffsetResponseData().setTopics(topics);
    ...
    public void testOffsetForLeaderEpochVersions() throws Exception
    {
        ...
        testAllMessageRoundTrips(new OffsetForLeaderEpochRequestData().setTopics(singletonList(new OffsetForLeaderEpochRequestData.OffsetForLeaderTopic().setName("foo").setPartitions(singletonList(partitionDataNoCurrentEpoch)))));
    ...
    public void testOffsetFetchVersions() throws Exception
    {
        ...
        testAllMessageRoundTrips(new OffsetFetchRequestData().setTopics(new ArrayList<>()).setGroupId(groupId));

        testAllMessageRoundTrips(new OffsetFetchRequestData().setGroupId(groupId).setTopics(topics));
    ...
    public void testProduceResponseVersions() throws Exception
    {
        ...
        testAllMessageRoundTrips(new ProduceResponseData().setResponses(singletonList(new ProduceResponseData.TopicProduceResponse().setName(topicName).setPartitions(singletonList(new ProduceResponseData.PartitionProduceResponse().setPartitionIndex(partitionIndex).setErrorCode(errorCode).setBaseOffset(baseOffset))))));

        Supplier<ProduceResponseData> response = () -> new ProduceResponseData().setResponses(singletonList(new ProduceResponseData.TopicProduceResponse().setName(topicName).setPartitions(singletonList(new ProduceResponseData.PartitionProduceResponse().setPartitionIndex(partitionIndex).setErrorCode(errorCode).setBaseOffset(baseOffset).setLogAppendTimeMs(logAppendTimeMs).setLogStartOffset(logStartOffset).setRecordErrors(singletonList(new ProduceResponseData.BatchIndexAndErrorMessage().setBatchIndex(batchIndex).setBatchIndexErrorMessage(batchIndexErrorMessage))).setErrorMessage(errorMessage))))).setThrottleTimeMs(throttleTimeMs);
    ...
    private void testByteBufferRoundTrip(short version, Message message, Message expected) throws Exception
    {
        ...
        assertEquals("The result of the size function does not match the number of bytes " + "written for version " + version, size, buf.position());
        Message message2 = message.getClass().getConstructor().newInstance();
        buf.flip();
        message2.read(byteBufferAccessor, version);
        assertEquals("The result of the size function does not match the number of bytes " + "read back in for version " + version, size, buf.position());
        assertEquals("The message object created after a round trip did not match for " + "version " + version, expected, message2);
    ...
    public void testMessageVersions()
    {
        for (ApiKeys apiKey : ApiKeys.values())
        {
            ...
            assertTrue("Request message spec for " + apiKey + " only " + "supports versions up to " + message.highestSupportedVersion(), apiKey.latestVersion() <= message.highestSupportedVersion());
            try
            {
            ...
            assertTrue("Response message spec for " + apiKey + " only " + "supports versions up to " + message.highestSupportedVersion(), apiKey.latestVersion() <= message.highestSupportedVersion());
        }
    }
    ...
    private static void compareTypes(NamedType typeA, NamedType typeB)
    {
        ...
        {
            throw new RuntimeException("Can't match up structures: typeA has " + Utils.join(listA, ", ") + ", but typeB has " + Utils.join(listB, ", "));
        }
        for (int i = 0; i < listA.size(); i++)
        {
            ...
            {
                compareTypes(new NamedType(entryA.name, entryA.type.arrayElementType().get()), new NamedType(entryB.name, entryB.type.arrayElementType().get()));
            }
        }
    }
    ...
    private void verifyWriteRaisesUve(short version, String problemText, Message message)
    {
        ...
        assertTrue("Expected to get an error message about " + problemText + ", but got: " + e.getMessage(), e.getMessage().contains(problemText));
    }
    ...
    private void verifyWriteSucceeds(short version, Message message)
    {
        ...
        assertEquals("Expected the serialized size to be " + size + ", but it was " + buf.position(), size, buf.position());
    }
