...
public class ApiMessageTypeTest
{
    ...
    public void testUniqueness()
    {
        ...
        for (ApiMessageType type : ApiMessageType.values())
        {
            assertFalse("found two ApiMessageType objects with id " + type.apiKey(), ids.contains(type.apiKey()));
            ids.add(type.apiKey());
            String requestName = type.newRequest().getClass().getSimpleName();
            assertFalse("found two ApiMessageType objects with requestName " + requestName, requestNames.contains(requestName));
            requestNames.add(requestName);
            String responseName = type.newResponse().getClass().getSimpleName();
            assertFalse("found two ApiMessageType objects with responseName " + responseName, responseNames.contains(responseName));
