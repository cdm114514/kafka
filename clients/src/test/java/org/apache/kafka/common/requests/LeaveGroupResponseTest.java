...
public class LeaveGroupResponseTest
{
    ...
    public void setUp()
    {
        memberResponses = Arrays.asList(new MemberResponse().setMemberId(memberIdOne).setGroupInstanceId(instanceIdOne).setErrorCode(Errors.UNKNOWN_MEMBER_ID.code()), new MemberResponse().setMemberId(memberIdTwo).setGroupInstanceId(instanceIdTwo).setErrorCode(Errors.FENCED_INSTANCE_ID.code()));
    }
    ...
    public void testConstructorWithStruct()
    {
        ...
        LeaveGroupResponseData responseData = new LeaveGroupResponseData().setErrorCode(Errors.NOT_COORDINATOR.code()).setThrottleTimeMs(throttleTimeMs);
    ...
    public void testConstructorWithMemberResponses()
    {
        ...
        for (short version = 0; version <= ApiKeys.LEAVE_GROUP.latestVersion(); version++)
        {
            LeaveGroupResponse leaveGroupResponse = new LeaveGroupResponse(memberResponses, Errors.NONE, throttleTimeMs, version);
            ...
            {
                assertEquals(Collections.singletonMap(Errors.UNKNOWN_MEMBER_ID, 1), leaveGroupResponse.errorCounts());
    ...
    public void testEqualityWithMemberResponses()
    {
        for (short version = 0; version <= ApiKeys.LEAVE_GROUP.latestVersion(); version++)
        {
            ...
            LeaveGroupResponse primaryResponse = new LeaveGroupResponse(localResponses, Errors.NONE, throttleTimeMs, version);

            // The order of members should not alter result data.
            Collections.reverse(localResponses);
            LeaveGroupResponse reversedResponse = new LeaveGroupResponse(localResponses, Errors.NONE, throttleTimeMs, version);
