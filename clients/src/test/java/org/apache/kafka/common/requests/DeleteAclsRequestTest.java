...
public class DeleteAclsRequestTest
{
    ...
    {
        final DeleteAclsRequest original = new DeleteAclsRequest.Builder(requestData(LITERAL_FILTER, PREFIXED_FILTER, ANY_FILTER)).build(V1);
        final Struct struct = original.toStruct();
    ...
    {
        return new DeleteAclsRequestData().setFilters(asList(acls).stream().map(DeleteAclsRequest::deleteAclsFilter).collect(Collectors.toList()));
    }
}
