...
public class SaslAuthenticatorTest
{
    ...
    {
        assertEquals("It is not possible to easily bump SASL_HANDSHAKE schema" + " due to improper version negotiation in clients < 2.5." + " Please see https://issues.apache.org/jira/browse/KAFKA-9577", ApiKeys.SASL_HANDSHAKE.latestVersion(), 1);
    }
    ...
    {
        ListOffsetResponseData data = new ListOffsetResponseData().setThrottleTimeMs(0).setTopics(Collections.singletonList(new ListOffsetTopicResponse().setName("topic").setPartitions(Collections.singletonList(new ListOffsetPartitionResponse().setErrorCode(Errors.NONE.code()).setLeaderEpoch(ListOffsetResponse.UNKNOWN_EPOCH).setPartitionIndex(0).setOffset(0).setTimestamp(0)))));
    ...
    public void testCannotReauthenticateAgainFasterThanOneSecond() throws Exception
    {
        ...
        } catch (AssertionError e)
        {
            ...
            assertTrue("Should have received the SaslHandshakeRequest bytes back since we re-authenticated too quickly, " + "but instead we got our generated message echoed back, implying re-auth succeeded when it " + "should not have: " + e, e.getMessage().matches(".*<\\[" + expectedResponseTextRegex + "]>.*<\\[" + receivedResponseTextRegex + ".*?]>"));
    ...
    private NioEchoServer startServerApiVersionsUnsupportedByClient(final SecurityProtocol securityProtocol, String saslMechanism) throws Exception
    {
        ...
        SaslChannelBuilder serverChannelBuilder = new SaslChannelBuilder(Mode.SERVER, jaasContexts, securityProtocol, listenerName, false, saslMechanism, true, credentialCache, null, time, new LogContext())
        {
            ...
            {
                return new SaslServerAuthenticator(configs, callbackHandlers, id, subjects, null, listenerName, securityProtocol, transportLayer, connectionsMaxReauthMsByMechanism, metadataRegistry, time)
                {
                    ...
                    protected ApiVersionsResponse apiVersionsResponse()
                    {
                        ...
                        for (ApiVersionsResponseKey apiVersion : defaultApiVersionResponse.data.apiKeys())
                        {
                            if (apiVersion.apiKey() != ApiKeys.SASL_AUTHENTICATE.id)
                            {
                                ...
                                apiVersions.add(new ApiVersionsResponseKey().setApiKey(apiVersion.apiKey()).setMinVersion(apiVersion.minVersion()).setMaxVersion(apiVersion.maxVersion()));
                            }
    ...
    private void createClientConnectionWithoutSaslAuthenticateHeader(final SecurityProtocol securityProtocol, final String saslMechanism, String node) throws Exception
    {
        ...
        SaslChannelBuilder clientChannelBuilder = new SaslChannelBuilder(Mode.CLIENT, jaasContexts, securityProtocol, listenerName, false, saslMechanism, true, null, null, time, new LogContext())
        {
    ...
    private void configureDigestMd5ServerCallback(SecurityProtocol securityProtocol)
    {
        ...
        saslServerConfigs.put(callbackPrefix + BrokerSecurityConfigs.SASL_SERVER_CALLBACK_HANDLER_CLASS, TestDigestLoginModule.DigestServerCallbackHandler.class);
    }
    ...
    {
        return NetworkTestUtils.createEchoServer(listenerName, securityProtocol, new TestSecurityConfig(saslServerConfigs), credentialCache, time);
    }
    ...
    {
        return NetworkTestUtils.createEchoServer(listenerName, securityProtocol, new TestSecurityConfig(saslServerConfigs), credentialCache, 100, time, tokenCache);
    }
                ...
                for (Callback callback : callbacks)
                {
                    if (callback instanceof OAuthBearerTokenCallback)
                    {
                        ...
                        if (token != null)
                        {
                            ...
                            try
                            {
                                claimsJson = String.format("{%s,%s,%s}", expClaimText(Long.parseLong(lifetimeSecondsValueToUse)), claimOrHeaderJsonText("iat", time.milliseconds() / 1000.0), claimOrHeaderJsonText("sub", changedPrincipalNameToUse));
    ...
    private static class AlternateSaslChannelBuilder extends SaslChannelBuilder
    {
        ...
        {
            super(mode, jaasContexts, securityProtocol, listenerName, isInterBrokerListener, clientSaslMechanism, handshakeRequestEnable, credentialCache, tokenCache, time, new LogContext());
        }
