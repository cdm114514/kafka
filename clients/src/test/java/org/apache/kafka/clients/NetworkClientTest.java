...
public class NetworkClientTest
{
    ...
    {
        return new NetworkClient(selector, metadataUpdater, "mock", Integer.MAX_VALUE, reconnectBackoffMsTest, reconnectBackoffMaxMs, 64 * 1024, 64 * 1024, defaultRequestTimeoutMs, connectionSetupTimeoutMsTest, connectionSetupTimeoutMaxMsTest, ClientDnsLookup.DEFAULT, time, true, new ApiVersions(), new LogContext());
    }
    ...
    {
        return new NetworkClient(selector, metadataUpdater, "mock-static", Integer.MAX_VALUE, 0, 0, 64 * 1024, 64 * 1024, defaultRequestTimeoutMs, connectionSetupTimeoutMsTest, connectionSetupTimeoutMaxMsTest, ClientDnsLookup.DEFAULT, time, true, new ApiVersions(), new LogContext());
    }

    private NetworkClient createNetworkClientWithNoVersionDiscovery(Metadata metadata)
    {
        return new NetworkClient(selector, metadata, "mock", Integer.MAX_VALUE, reconnectBackoffMsTest, 0, 64 * 1024, 64 * 1024, defaultRequestTimeoutMs, connectionSetupTimeoutMsTest, connectionSetupTimeoutMaxMsTest, ClientDnsLookup.DEFAULT, time, false, new ApiVersions(), new LogContext());
    }

    private NetworkClient createNetworkClientWithNoVersionDiscovery()
    {
        return new NetworkClient(selector, metadataUpdater, "mock", Integer.MAX_VALUE, reconnectBackoffMsTest, reconnectBackoffMaxMsTest, 64 * 1024, 64 * 1024, defaultRequestTimeoutMs, connectionSetupTimeoutMsTest, connectionSetupTimeoutMaxMsTest, ClientDnsLookup.DEFAULT, time, false, new ApiVersions(), new LogContext());
    }
    ...
    private void checkSimpleRequestResponse(NetworkClient networkClient)
    {
        ...
        ProduceRequest.Builder builder = new ProduceRequest.Builder(PRODUCE.latestVersion(), PRODUCE.latestVersion(), (short) 1, 1000, Collections.emptyMap(), null);
        ...
        ResponseHeader respHeader = new ResponseHeader(request.correlationId(), request.apiKey().responseHeaderVersion(PRODUCE.latestVersion()));
    ...
    public void testApiVersionsRequest()
    {
        ...
        // prepare response
        delayedApiVersionsResponse(0, ApiKeys.API_VERSIONS.latestVersion(), ApiVersionsResponse.DEFAULT_API_VERSIONS_RESPONSE);
    ...
    public void testUnsupportedApiVersionsRequestWithVersionProvidedByTheBroker()
    {
        ...
        apiKeys.add(new ApiVersionsResponseKey().setApiKey(ApiKeys.API_VERSIONS.id).setMinVersion((short) 0).setMaxVersion((short) 2));
        ...
        // prepare response
        delayedApiVersionsResponse(1, (short) 0, ApiVersionsResponse.DEFAULT_API_VERSIONS_RESPONSE);
    ...
    public void testConnectionSetupTimeout()
    {
        ...
        client.poll(0, time.milliseconds());
        assertFalse("The connections should not fail before the socket connection setup timeout elapsed", client.connectionFailed(node));

        time.sleep((long) (connectionSetupTimeoutMsTest * 1.2) + 1);
        client.poll(0, time.milliseconds());
        assertTrue("Expected the connections to fail due to the socket connection setup timeout", client.connectionFailed(node));
    }
    ...
    private ApiVersionsResponse createExpectedApiVersionsResponse(ApiKeys key, short maxVersion)
    {
        ...
        for (ApiKeys apiKey : ApiKeys.values())
        {
            if (apiKey == key)
            {
                versionList.add(new ApiVersionsResponseKey().setApiKey(apiKey.id).setMinVersion((short) 0).setMaxVersion(maxVersion));
            } else
            {
                versionList.add(new ApiVersionsResponseKey().setApiKey(apiKey.id).setMinVersion(apiKey.oldestVersion()).setMaxVersion(apiKey.latestVersion()));
            }
        }
