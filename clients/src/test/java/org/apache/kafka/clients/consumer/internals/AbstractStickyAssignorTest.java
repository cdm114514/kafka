...
public abstract class AbstractStickyAssignorTest
{
    ...
    public void testOnlyAssignsPartitionsFromSubscribedTopics()
    {
        ...
        subscriptions = mkMap(mkEntry(consumerId, buildSubscription(topics(topic), Arrays.asList(tp(topic, 0), tp(topic, 1), tp(otherTopic, 0), tp(otherTopic, 1)))));
    ...
    public void testMoveExistingAssignments()
    {
        ...
        subscriptions.put("consumer01", buildSubscription(topics("topic01", "topic02"), partitions(tp("topic01", 0))));
        subscriptions.put("consumer02", buildSubscription(topics("topic01", "topic02", "topic03", "topic04"), partitions(tp("topic02", 0), tp("topic03", 0))));
        subscriptions.put("consumer03", buildSubscription(topics("topic02", "topic03", "topic04", "topic05", "topic06"), partitions(tp("topic04", 0), tp("topic05", 0), tp("topic06", 0))));
    ...
    public void testStickiness()
    {
        ...
        subscriptions.remove(consumer1);
        subscriptions.put(consumer2, buildSubscription(topics("topic01"), assignment.get(consumer2)));
        subscriptions.put(consumer3, buildSubscription(topics("topic01"), assignment.get(consumer3)));
        subscriptions.put(consumer4, buildSubscription(topics("topic01"), assignment.get(consumer4)));
     ...
     */
    protected void verifyValidityAndBalance(Map<String, Subscription> subscriptions, Map<String, List<TopicPartition>> assignments, Map<String, Integer> partitionsPerTopic)
    {
